...
export default function ImageProject({ title, description, image, portrait, url, priority = false }: ImageProjectProps) {
  return (
    <article className={`group relative rounded-lg shadow-lg overflow-hidden max-w-full w-full transition-transform duration-200 hover:-translate-y-1 ${portrait ? "h-96" : "h-64"}`}>
      ...
      >
        ...
        <div className="relative z-10 text-left text-white">
          <h2 className="text-2xl md:text-3xl font-bold drop-shadow-md">{title}</h2>
          {description && (
